...
tile_size = 8 # one divided region pixel for original image 
...
def matchRGB(current_rgb, json):
	...
	for tile in json:
		d = 0
		# url = tile[u'photo'][u'urls'][u'url'][0][u'url_q']
	 ...
	 #   	image = img.convert('RGB')
		...
		# tile_rgb = getAverageColor2(image, img_size)
		...
		if d  < dis:
			tile_now = tile  # update the tile if the distance is smaller than the previous one
			dis = d
		else:
			pass
...
im = Image.open("original_Image/bear.jpg") # read attribution of original image
...
with open('Mosaic_Tiles_Library/TilesLibrary_16_x_16.json') as data_file: 
	data = json.load(data_file)
#write new tiles json for the front use.
t = open('Outcome_tiles_library_For_Front/BearFinalTiles.json', 'w')
t.write('[')
